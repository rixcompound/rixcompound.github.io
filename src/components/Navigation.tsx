...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // Apply transforms directly via native SVG attributes for flawless iOS Safari/WebKit compliance
      if (bigSprocketRef.current) {
        bigSprocketRef.current.setAttribute("transform", `rotate(${bigAngle}, 24, 24)`);
      }
      if (smallSprocketRef.current) {
        smallSprocketRef.current.setAttribute("transform", `rotate(${smallAngle}, 94.5, 24)`);
      }
      if (chainRef.current) {
